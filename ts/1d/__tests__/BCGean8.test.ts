...
const defaultText = '1234567',
  secondText = '0987654';
describe('Ean8', function () {
  let instance: BCGean8;
  ...
  describe('#maxSize', function () {
    describe('with no label', function () {
      it('should work with scale=1', function () {
        ...
        instance.parse(defaultText);
        deepStrictEqual(instance.getDimension(0, 0), [(8 * defaultText.length + 3 + 5 + 3) * scale, 30 * scale]);
      });
      ...
      it('should work with scale=2 and extra positioning', function () {
        ...
        instance.setThickness(30);
        instance.parse(defaultText);
        deepStrictEqual(instance.getDimension(left, top), [(8 * defaultText.length + 3 + 5 + 3 + left) * scale, (30 + top) * scale]);
      });
      ...
      it('should work with scale=2 and extra positioning and offset', function () {
        ...
        deepStrictEqual(instance.getDimension(left, top), [
          (8 * defaultText.length + 3 + 5 + 3 + left + offsetX) * scale,
    ...
    describe('with one label', function () {
      it('should work with scale=1', function () {
        ...
        // We should test Y
        strictEqual(instance.getDimension(0, 0)[0], (8 * defaultText.length + 3 + 5 + 3) * scale);
      });
